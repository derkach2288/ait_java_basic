/*
Дан список Programmer(String name, String city, List<Task>  tasks). Каждый программист  имеет список задач Task
(int Number, String description, Status status). Естественно,  Status это enum.  Сформировать список из всех незавершенных задач (т.е. имеют статус, отличный от «DONE») , которые относятся к программистам из Берлина.
